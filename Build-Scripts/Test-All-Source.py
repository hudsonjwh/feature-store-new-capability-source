...
# MAGIC 
# MAGIC Any lingering state can be addressed by properly configuring [Reset-Project]($./Reset-Project) to drop databases, tables or delete on-disk artifacts
...
client = DBAcademyRestClient(throttle=1)
...
def execute_tests():
  ...
  round_1.add(f"{home}/Source/Includes/Reset")
  round_1.add(f"{home}/Source/Version Info")

  round_2 = dbtest.SuiteBuilder(client, course_name, test_type="Source")
  round_2.add(f"{home}/Source/EC 01 - Your First Lesson")
  round_2.add(f"{home}/Source/EC 02 - Your Second Lesson")
...
displayHTML(evaluator.to_html(print_success_links=keep_success))
